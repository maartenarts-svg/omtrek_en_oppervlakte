...
function init72FormulasDrill(container, onComplete) {
    addCSS72();
    ...
    const DRAG_ZONES = [
        ...
        { id: 'fA3', label: '<i>bh</i> : 2',           correct: ['A_driehoek'] },
        { id: 'fA4', label: 'π<i>r</i>²',             correct: ['A_cirkel'] }
    ];
    ...
    const DRAG_ITEMS = [
        ...
        { id: 'A_driehoek',       label: '<i>A</i><sub>driehoek</sub>' },
        { id: 'A_cirkel',         label: '<i>A</i><sub>cirkel</sub>' }
    ];
    ...
    const FORMULA_OPTIONS = [
        ...
        { id: 'fA1', label: '<i>bh</i>' },
        { id: 'fA2', label: '<i>z</i>²' },
        { id: 'fA3', label: '<i>bh</i> : 2' },
        { id: 'fA4', label: 'π<i>r</i>²' }
    ];
    ...
    const VRAAG_SPECS = shuffle([
        ...
        { type: 'cirkel',                letter: '<i>A</i>', correctId: 'fA4' },
        { type: 'driehoek-hoogte',       letter: '<i>P</i>', correctId: 'f3'  },
        { type: 'driehoek-hoogte',       letter: '<i>A</i>', correctId: 'fA3' },
    ...
    let q16Attempt1Results = null;
    let q17Attempt1Results = null;
    ...
    function render() {
        ...
        else if (currentQuestion <= 14)  renderDropdownQ(currentQuestion - 2);
        else if (currentQuestion === 15) renderQ15();
        else if (currentQuestion === 16) renderQ16();
        else if (currentQuestion === 17) renderQ17();
        else                             finish();
    }
        ...
        container.innerHTML = `
            <div class="exercise-container">
                ...
                <div class="question-card">
                    ...
                    <div class="items-pool ex72-pool" id="shapesPool">
    ...
    function checkQ15() {
        ...
        const results = { z: vals.z === 'zijde', b: vals.b === 'basis', h: vals.h === 'hoogte', r: vals.r === 'straal' };
        if (q15Attempt1Results === null) {
            ...
            let pts = 0;
            if (q15Attempt1Results.z) pts += 1; else if (results.z) pts += 0.5;
            if (q15Attempt1Results.b) pts += 1; else if (results.b) pts += 0.5;
            if (q15Attempt1Results.h) pts += 1; else if (results.h) pts += 0.5;
            if (q15Attempt1Results.r) pts += 1; else if (results.r) pts += 0.5;
            totalPoints += pts;
            ...
            showFeedbackWithNext(allOk ? 'correct' : 'incorrect',
                allOk ? 'Correct bij de tweede poging.' : 'Niet helemaal juist. De juiste antwoorden zijn ingevuld.');
        }
    }

    // ── Q16: LETTERS OPPERVLAKTE ─────────────────────────────
    function renderQ16() {
        q16Attempt1Results = null;
        container.innerHTML = `
            <div class="exercise-container">
                ${progressHTML()}
                <div class="question-card">
                    <h3 class="question-title">Wat betekenen de letters?</h3>
                    <p class="question-text">
                        In de formules <i>A</i> = <i>bh</i>, <i>A</i> = <i>z</i>²,
                        <i>A</i> = <i>bh</i> : 2 en <i>A</i> = π<i>r</i>²
                        staat de letter <i>A</i> voor oppervlakte.<br>
                        Waarvoor staan de letters <i>b</i>, <i>h</i>, <i>z</i> en <i>r</i>?
                    </p>
                    <div class="letter-inputs">
                        <div class="letter-input-row">
        ...
        document.getElementById('checkBtn').addEventListener('click', checkQ16);
    }
    ...
    function checkQ16() {
        ...
        if (!vals.b || !vals.h || !vals.z || !vals.r) {
            showFeedback('incorrect', 'Vul alle vakken in.');
            return;
        }
        const results = { b: vals.b === 'basis', h: vals.h === 'hoogte', z: vals.z === 'zijde', r: vals.r === 'straal' };
        if (q16Attempt1Results === null) {
            if (Object.values(results).every(Boolean)) {
                totalPoints += 4;
                lockInputs(['inputB','inputH','inputZ','inputR']);
                document.getElementById('checkBtn').style.display = 'none';
                showFeedbackWithNext('correct', 'Correct!');
            } else {
                q16Attempt1Results = { ...results };
                showFeedback('incorrect', 'Dit klopt niet helemaal. Verbeter.');
            }
        } else {
            let pts = 0;
            if (q16Attempt1Results.b) pts += 1; else if (results.b) pts += 0.5;
            if (q16Attempt1Results.h) pts += 1; else if (results.h) pts += 0.5;
            if (q16Attempt1Results.z) pts += 1; else if (results.z) pts += 0.5;
            if (q16Attempt1Results.r) pts += 1; else if (results.r) pts += 0.5;
            totalPoints += pts;
            if (!results.b) document.getElementById('inputB').value = 'basis';
            if (!results.h) document.getElementById('inputH').value = 'hoogte';
            if (!results.z) document.getElementById('inputZ').value = 'zijde';
    ...
    // ── Q17: FORMULES AANVULLEN ──────────────────────────────
    function renderQ17() {
        q17Attempt1Results = null;
        const FIELDS = [
            ...
            { id: 'inDriehoekA',       label: '<i>A</i><sub>driehoek</sub> =' },
            { id: 'inCirkelA',         label: '<i>A</i><sub>cirkel</sub> =' }
        ];
        ...
        container.innerHTML = `
            <div class="exercise-container">
                ...
                <div class="question-card">
                    ...
                    <p class="hint-text">Tip: druk <kbd>*</kbd> voor het maalteken · &nbsp;|&nbsp; typ <kbd>pi</kbd> voor π &nbsp;|&nbsp; gebruik <kbd>/</kbd> of <kbd>:</kbd> voor de deling</p>
                    <div id="feedbackArea" class="feedback-area"></div>
                    <div class="question-actions">
                        <button class="btn btn-primary" id="checkBtn">Controleer</button>
                    </div>
        ...
        document.getElementById('checkBtn').addEventListener('click', checkQ17);
    }

    function normalizeFormula(str) {
        return str.replace(/pi/gi, 'π').replace(/·/g, '').replace(/\s+/g, '');
    }
    ...
    function checkQ17() {
        const IDS = ['inVierkantP','inRechthoekP','inRuitP','inCirkelP','inRechthoekA','inVierkantA','inParallellogramA','inDriehoekA','inCirkelA'];
        const vals = {};
        ...
        const CORRECT_VALS = {
            inVierkantP: '4z', inRechthoekP: '2(b+h)', inRuitP: '4z', inCirkelP: '2πr',
            inRechthoekA: 'bh', inVierkantA: 'z²', inParallellogramA: 'bh',
            inDriehoekA: 'bh:2', inCirkelA: 'πr²'
        };
        ...
        const results = {
            ...
            inDriehoekA:       matchesFormula(vals.inDriehoekA,       ['bh:2','hb:2','bh/2','hb/2']),
            inCirkelA:         matchesFormula(vals.inCirkelA,         ['πr²','r²π'])
        };

        if (q17Attempt1Results === null) {
            if (Object.values(results).every(Boolean)) {
                totalPoints += 9;
                lockInputs(IDS);
                document.getElementById('checkBtn').style.display = 'none';
                ...
                q17Attempt1Results = { ...results };
                showFeedback('incorrect', 'Dit klopt niet helemaal. Verbeter.');
            }
        } else {
            let points = 0;
            IDS.forEach(id => {
                if (q17Attempt1Results[id]) points += 1;
            ...
            document.getElementById('checkBtn').style.display = 'none';
            const allOk = Object.values(results).every(Boolean);
            showFeedbackWithFinish(allOk ? 'correct' : 'incorrect',
                allOk ? 'Correct bij de tweede poging.' : 'Niet helemaal juist. De juiste antwoorden zijn ingevuld.');
        }
    }
    ...
    // ── CSS ──────────────────────────────────────────────────
    function addCSS72() {
        if (['ex58-style','ex63-style','ex72-style'].some(id => document.getElementById(id))) return;
        const s = document.createElement('style');
        s.id = 'ex72-style';
        s.textContent = `
.ex58-zones-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: var(--spacing-md); }
.ex58-drop-zone { min-height: 80px; border: 2px dashed var(--color-gray,#ccc); border-radius: var(--radius-md,8px); padding: var(--spacing-sm); }
.ex72-pool .pool-items { grid-template-columns: repeat(auto-fill, minmax(130px,1fr)); gap: var(--spacing-sm); }
.ex72-pool .draggable-item { min-height: 36px; padding: 0.25rem 0.4rem; font-size: var(--font-size-small,0.85rem); }
.ex72-pool .pool-title { margin-bottom: var(--spacing-sm); }
.ex58-formula-row { display: flex; align-items: center; gap: var(--spacing-md); margin: var(--spacing-lg) 0; }
.ex58-letter { font-size: 1.1rem; white-space: nowrap; }
.ex58-cs { position: relative; display: inline-block; min-width: 210px; }
.ex58-cs-display { padding: 0.35rem 0.6rem; border: 2px solid var(--color-gray,#ccc); border-radius: var(--radius-md,6px); cursor: pointer; background: #fff; font-size: var(--font-size-base,0.95rem); min-height: 1.8rem; display: flex; align-items: center; user-select: none; }
.ex58-cs-display:hover { border-color: var(--color-primary,#4a7a10); }
.ex58-cs-list { position: absolute; top: calc(100% + 2px); left: 0; right: 0; background: #fff; border: 2px solid var(--color-primary,#4a7a10); border-radius: var(--radius-md,6px); z-index: 200; box-shadow: 0 4px 12px rgba(0,0,0,.15); }
.ex58-cs-opt { padding: 0.45rem 0.6rem; cursor: pointer; font-size: var(--font-size-base,0.95rem); min-height: 1.8rem; display: flex; align-items: center; }
.ex58-cs-opt:hover { background: var(--color-light,#f0f7e0); }
.formula-title { font-size: var(--font-size-base,0.95rem); font-weight: 700; padding-bottom: var(--spacing-sm); border-bottom: 2px solid var(--color-gray); margin-bottom: var(--spacing-sm); }
.figure-container { display: flex; justify-content: center; align-items: center; margin: var(--spacing-lg) 0; min-height: 200px; }
...
.letter-input { flex: 1; padding: var(--spacing-sm) var(--spacing-md); font-size: var(--font-size-large); border: 2px solid var(--color-gray); border-radius: var(--radius-md); }
...
.letter-example { flex: 1; padding: var(--spacing-sm) var(--spacing-md); font-size: var(--font-size-large); background: var(--color-light); border: 2px solid var(--color-gray); border-radius: var(--radius-md); color: #555; font-style: italic; }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { init72FormulasDrill };
}
