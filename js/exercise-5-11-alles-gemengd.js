...
function init511AllesGemengd(container, onComplete) {
    ...
    // ── VOORBEELD (vierkant oppervlakte: z=5 cm, A=25 cm²) ───
    ...
    function renderExample() {
        container.innerHTML = `
            <div class="exercise-container">
                <div class="question-card">
                    ...
                    <div class="ex33-fig" id="ex511-fig-eg"></div>
        ...
        drawFiguur(document.getElementById('ex511-fig-eg'), 'vierkant', {
            factor: 1, rotation: 0,
    ...
    function renderQuestion(n) {
        ...
        container.innerHTML = `
            <div class="exercise-container">
                ...
                <div class="question-card">
                    ...
                    <p class="ex54-subtitle">Je mag ICT gebruiken voor de berekening.</p>
                    <div class="ex33-fig" id="ex511-fig"></div>
                    <div class="ex33-rows">
                        <div class="ex33-row">
                            <span class="ex33-label">Formule:</span>
                            <span class="ex33-p-label">${pLabel} =</span>
                            <div class="ex33-cs" id="ex511-cs" data-value="">
                        ...
                        <div class="ex33-row ex33-row-calc">
                            <span class="ex33-label">Berekening in</span>
                            <select id="ex511-unit" class="ex33-unit">${unitOptsHtml}</select>
                            <span>:</span>
                            <span class="ex33-p-label">${pLabel} =</span>
                            <input id="ex511-calc" class="ex33-calc" type="text" autocomplete="off" placeholder="berekening">
                            <span>${calcSep}</span>
                            <input id="ex511-ans" class="ex33-ans" type="text" autocomplete="off" placeholder="antwoord">
                        </div>
                        <div class="ex33-row ex33-sentence">
                            Antwoord: De ${sentenceWord} is
                            <span id="ex511-ans-disp" class="ex33-val">...</span>
                            <span id="ex511-unit-disp" class="ex33-val">...</span>.
                        </div>
                    </div>
                    ...
                    <p class="hint-text">Tip: typ <kbd>*</kbd> voor het maalteken &middot; &nbsp;|&nbsp; typ <kbd>pi</kbd> voor &pi;</p>
        ...
        const figType = data.fig === 'parallellogram' && data.task === 'oppervlakte'
            ...
            : data.fig;
        drawFiguur(document.getElementById('ex511-fig'), figType, data.figOpts);
        initCS('ex511-cs');

        const ansEl  = document.getElementById('ex511-ans');
        const unitEl = document.getElementById('ex511-unit');
        const calcEl = document.getElementById('ex511-calc');

        function updateRow3() {
            document.getElementById('ex511-ans-disp').textContent  = ansEl.value.trim()  || '...';
            document.getElementById('ex511-unit-disp').textContent = unitEl.value || '...';
        }
        ...
        document.getElementById('checkBtn').addEventListener('click', () => {
            qAttempts++;
            const formula = document.getElementById('ex511-cs').dataset.value || '';
    ...
    function fillCorrectAnswer(data) {
        const cs      = document.getElementById('ex511-cs');
        ...
        document.getElementById('ex511-unit').value = correctUnit;

        document.getElementById('ex511-calc').value = buildCalcCorrect(data);

        const ansStr = String(data.answer);
        document.getElementById('ex511-ans').value             = ansStr;
        document.getElementById('ex511-ans-disp').textContent  = ansStr;
        document.getElementById('ex511-unit-disp').textContent = correctUnit;
    }
    ...
    // ── CSS ──────────────────────────────────────────────────
    // ex59-style en ex510-style bevatten geen squared-helper CSS → niet voldoende als guard.

    function addCSS511() {
        if (['ex33-style', 'ex36-style', 'ex54-style', 'ex511-style'].some(id => document.getElementById(id))) return;
