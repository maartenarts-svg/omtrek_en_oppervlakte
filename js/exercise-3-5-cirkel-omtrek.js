// ============================================
// OEFENING 3-5: FORMULES DRILL MET CIRKEL
// 11 vragen over omtrekformules (inclusief cirkel)
// ============================================

function init35CirkelOmtrek(container, onComplete) {
    addCSS35();
    ...
    const DRAG_FORMULAS = [
        ...
        { id: 'f4', label: '<i>P</i> = 2π<i>r</i>',              correct: ['cirkel'] }
    ];

    const FORMULA_OPTIONS = [
        { id: 'f1', label: '<i>P</i> = 4<i>z</i>' },
        { id: 'f2', label: '<i>P</i> = 2(<i>b</i> + <i>h</i>)' },
        { id: 'f4', label: '<i>P</i> = 2π<i>r</i>' },
    ...
    function render() {
        ...
        else if (currentQuestion <= 9)   renderRadioQ(currentQuestion - 2);
        else if (currentQuestion === 10) renderQ10();
        else if (currentQuestion === 11) renderQ11();
    }
    ...
    // ============================================
    // Q1: DRAG & DROP (8 figuren, 4 formules)
    // ============================================
    function renderQ1() {
        q1Attempts = 0;
        container.innerHTML = `
            <div class="exercise-container">
                ...
                <div class="question-card">
                    ...
                    <div class="drop-zones ex35-formulas-grid" style="margin-top: var(--spacing-xl);">
                        ${DRAG_FORMULAS.map(f => `
        ...
        if (allCorrect) {
            ...
            document.getElementById('feedbackArea').innerHTML = `
                <div class="feedback-message feedback-incorrect">
                    ...
                    <div class="correct-answer-display ex35-formulas-grid" style="margin-top: var(--spacing-md);">${cols}</div>
    ...
    // ============================================
    // Q2-9: FIGUUR + RADIOBUTTONS (8 shapes)
    // ============================================
            ...
            <div class="exercise-container">
                ...
                <div class="question-card">
                    ...
                    <p class="question-text">
                        In de formules <i>P</i> = 4<i>z</i>, <i>P</i> = 2(<i>b</i> + <i>h</i>) en <i>P</i> = 2π<i>r</i> staat de letter <i>P</i> voor omtrek.<br>
    ...
    function checkQ10() {
        ...
        if (q10Attempt1Results === null) {
            ...
            let points = 0;
            if (q10Attempt1Results.z) points += 1; else if (results.z) points += 0.5;
            if (q10Attempt1Results.b) points += 1; else if (results.b) points += 0.5;
            if (q10Attempt1Results.h) points += 1; else if (results.h) points += 0.5;
            if (q10Attempt1Results.r) points += 1; else if (results.r) points += 0.5;
            totalPoints += points;
    ...
    // ============================================
    // Q11: FORMULES AANVULLEN (inclusief cirkel)
    // ============================================
    function renderQ11() {
        q11Attempt1Results = null;
        container.innerHTML = `
            <div class="exercise-container">
                ${progressHTML()}
                <div class="question-card">
                    ...
                    <div class="letter-inputs">
                        ...
                        <div class="letter-input-row">
                            <label class="letter-label formula-label"><i>P</i><sub>cirkel</sub> =</label>
                            <input type="text" id="inputCirkel" class="letter-input" autocomplete="off">
                        </div>
                    </div>
                    <p class="hint-text">Tip: druk * voor het maalteken · &nbsp;|&nbsp; typ "pi" voor π</p>
                    <div id="feedbackArea" class="feedback-area"></div>
                    <div class="question-actions">
                        <button class="btn btn-primary" id="checkBtn">Controleer</button>
                    </div>
        ...
        ['inputVierkant', 'inputRechthoek', 'inputRuit', 'inputCirkel'].forEach(id => {
            const el = document.getElementById(id);
            ...
            el.addEventListener('input', () => {
                ...
                if (newVal !== old) {
                    ...
                    el.value = newVal;
                    el.selectionStart = el.selectionEnd = Math.max(0, cursor - piCount);
                }
            });
        });

        document.getElementById('checkBtn').addEventListener('click', checkQ11);
    }
    ...
    function checkQ11() {
        const vals = {
            ...
            cirkel:    document.getElementById('inputCirkel').value
        };
        if (!vals.vierkant.trim() || !vals.rechthoek.trim() || !vals.ruit.trim() || !vals.cirkel.trim()) {
            showFeedback('incorrect', 'Vul alle vakken in.');
            return;
        }
        ...
        const results = {
            ...
            cirkel:    matchesFormula(vals.cirkel,     ['2πr', '2rπ'])
        };

        if (q11Attempt1Results === null) {
            if (results.vierkant && results.rechthoek && results.ruit && results.cirkel) {
                totalPoints += 4;
                lockInputs(['inputVierkant', 'inputRechthoek', 'inputRuit', 'inputCirkel']);
                document.getElementById('checkBtn').style.display = 'none';
                showFeedbackWithFinish('correct', 'Correct!');
            } else {
                q11Attempt1Results = { ...results };
                showFeedback('incorrect', 'Dit klopt niet helemaal. Verbeter.');
            }
        } else {
            let points = 0;
            if (q11Attempt1Results.vierkant)  points += 1; else if (results.vierkant)  points += 0.5;
            if (q11Attempt1Results.rechthoek) points += 1; else if (results.rechthoek) points += 0.5;
            if (q11Attempt1Results.ruit)      points += 1; else if (results.ruit)      points += 0.5;
            if (q11Attempt1Results.cirkel)    points += 1; else if (results.cirkel)    points += 0.5;
            totalPoints += points;
            ...
            if (!results.cirkel)    document.getElementById('inputCirkel').value    = '2πr';

            lockInputs(['inputVierkant', 'inputRechthoek', 'inputRuit', 'inputCirkel']);
            document.getElementById('checkBtn').style.display = 'none';
            const allOk = results.vierkant && results.rechthoek && results.ruit && results.cirkel;
    ...
    // ============================================
    function addCSS35() {
        if (document.getElementById('formules-drill-35-css')) return;
        const style = document.createElement('style');
        style.id = 'formules-drill-35-css';
        style.textContent = `
            .ex35-formulas-grid {
                display: grid;
            ...
            @media (max-width: 768px) {
                .ex35-formulas-grid { grid-template-columns: repeat(2, 1fr); }
            }
            @media (max-width: 480px) {
                .ex35-formulas-grid { grid-template-columns: 1fr; }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { init35CirkelOmtrek };
}
